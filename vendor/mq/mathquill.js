...
JS environment could actually contain many instances. */
    ...
    var Controller = P(function(_) {
        ...
        _.notify = function() {
            ...
            if (this.root.onNotify) {
                this.root.onNotify(this.cursor, arguments);
            }
            return this;
        };
    });
  ...
  */
        ...
        _.html = function() {
            ...
            if (cmd.ctrlSeq === ":") {
                cmd.htmlTemplate = "<span class='colon'>:</span>";
            }
    ...
    var TextBlock = P(Node, function(_, super_) {
        ...
        _.latex = function() {
            ...
            return (
                "\\text{" + contents.replace(/\\/g, "\\backslash ").replace(/[{}]/g, "\\$&") + "}"
            );
        };
        _.html = function() {
            return (
    ...
    var SupSub = P(MathCommand, function(_, super_) {
        _.ctrlSeq = "_{...}^{...}";
        _.createLeftOf = function(cursor) {
            if (!this.replacedFragment && !cursor[L] && cursor.options.supSubsRequireOperand)
                return;
    ...
    LatexCmds.superscript = LatexCmds.supscript = LatexCmds["^"] = P(SupSub, function(_, super_) {
        ...
        _.reflow = function() {
            ...
            $block.css("vertical-align", String(h + 4) + "px");
        };
    });
    ...
    var SquareRoot = (LatexCmds.sqrt = LatexCmds["\u221a"] = P(MathCommand, function(_, super_) {
        ...
        _.reflow = function() {
            ...
            var parentLabel = block.closest(".math-label");
            var scaleY = (block.innerHeight() / +block.css("fontSize").slice(0, -2)) * 1.05;
            // console.log('scaleY', scaleY, block, block.innerHeight(), block.css('fontSize'), parentLabel.css('fontSize'));
            var correctYPos = (-0.07 * 1) / scaleY;
            block
                .prev()
                .css(
                    transformPropName,
                    "scale(" + 1 + "," + scaleY + ") translate(0, " + correctYPos + "em)"
                );
    ...
    function DelimsMixin(_, super_) {
        ...
        _.reflow = function() {
            ...
            // FIXME: SZLACHLA
            var lineHeight = 28;
            var vAlign = height * lineHeight - lineHeight;
            var dynamicFracAlign = this.jQ.closest(".mq-root-block").data("dynamicfrac");
            // console.log('dynFrac', height, dynamicFracAlign);
            if (dynamicFracAlign && height > dynamicFracAlign) {
                // console.log('SETTING');
                this.jQ.css("vertical-align", String(0.3 * vAlign - 10) + "px");
            }
            this.delimjQs.css(
                transformPropName,
                "scale(" +
                    min(1 + 0.2 * (height - 1), 1.2) +
                    "," +
                    1.2 * height +
                    ") translate(0, -0.1em)"
            );
            ...
            //this.delimjQs.get(1).style.transformOrigin = 'center ' + height + 'px';
        };
    }
    ...
    var Bracket = P(P(MathCommand, DelimsMixin), function(_, super_) {
        ...
        _.html = function() {
            ...
            this.htmlTemplate = // be set by createLeftOf or parser
                '<span class="mq-non-leaf mq-brackets-wrapper">' +
    ...
    LatexCmds.editable = LatexCmds.MathQuillMathField = P(MathCommand, function(_, super_) { // backcompat with before cfd3620 on #233
...
*/
    ...
    LatexCmds.br = bind(VanillaSymbol, "\\br ", "<br/>");
        ...
        _.text = function() {
            ...
            if (this.isPartOfOperator) {
                ...
                if (
                    ...
                    text = "*" + text;
                if (this[R] && !(this[R] instanceof BinaryOperator) && !(this[R] instanceof SupSub))
                    text += "*";
            }
            return text;
        };
    ...
    (function() {
        ...
        for (var i = 0; i < autoTrigs.length; i += 1) {
            AutoOpNames[autoTrigs[i]] = AutoOpNames["arc" + autoTrigs[i]] = AutoOpNames[
                autoTrigs[i] + "h"
            ] = AutoOpNames["ar" + autoTrigs[i] + "h"] = AutoOpNames[
                "arc" + autoTrigs[i] + "h"
            ] = 1;
        }
